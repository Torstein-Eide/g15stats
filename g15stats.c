...
static void draw_optional_screen(g15canvas *canvas, char *tmpstr, int screen,
...
static void run_loop(g15canvas *canvas, char *tmpstr, int unicore,
    ...
    while (keep_running) {
        ...
        switch (cycle) {
            ...
            case SCREEN_GPU:
            case SCREEN_MEM_PRESSURE: {
                /* Redirect if screen is unavailable on arrival (e.g. forced_screen
                 * pointing at a disabled feature). Normal key navigation already
                 * calls find_next_visible_screen, so this only fires in edge cases. */
                if (!screen_is_visible(cycle)) {
                    cycle = find_next_visible_screen(cycle, 1);
                    info_cycle = cycle;
                }
                draw_optional_screen(canvas, tmpstr, cycle, unicore, (char *) interface);
                /* draw_bat_screen / draw_g15_stats_info_screen may set their
                 * have_* flag to 0 mid-call (sensor lost). Redirect if so. */
                if (!screen_is_visible(cycle)) {
                    cycle = find_next_visible_screen(cycle, 1);
                    info_cycle = cycle;
                    draw_optional_screen(canvas, tmpstr, cycle, unicore, (char *) interface);
